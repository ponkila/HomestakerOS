...
pub fn run_json2nix(json_str: &str) -> Result<String, String> {
    ...
    {
        let stdin = child.stdin.as_mut().ok_or_else(|| "Failed to open stdin for json2nix".to_string())?;
...
pub fn write_default_nix(hostname_dir: &Path, json2nix_output: &str) -> std::io::Result<()> {
    let nix_boilerplate = format!("{{ pkgs, lib, config, ... }}: {{ homestakeros = {}; }}", json2nix_output);
...
pub fn run_nix_build(nix_config_dir: &Path, hostname: &str, out_link: &Path) -> Result<(), String> {
    ...
    println!("Nix build stdout: {}", String::from_utf8_lossy(&output.stdout));
    println!("Nix build stderr: {}", String::from_utf8_lossy(&output.stderr));
    Ok(())
}

/// Create a tar archive from a source directory.
pub fn create_tarball<P: AsRef<Path>, Q: AsRef<Path>>(source: P, dir_name: &str, tar_path: Q) -> std::io::Result<()> {
